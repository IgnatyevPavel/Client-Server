...
___
### 39. уровни модели OSI 

![OSI](img\OSI-TCPIP_lvls.JPG)

OSI (Open Systems Interconnection) – это модель состоящая из набора правил, созданная для обмена данными между различными устройствами в сети , состоящая из 7 уровней. цели каждого уровня абстрагированы от других уровней
...
прокси сервер это посредник между клиентом и сервером. прокси может использоваться как в интересах клиента, если он хочет не показывать своё местоположение по ip адресу(если доступ к ресурсу был ограничен), так и в интересах сервера, что повышает отказоустойчивость системы, так как прокси сервер может использоваться как дополнительный фильтр который 
Прокси-сервер защищает сервер, фильтруя трафик, блокируя нежелательные запросы, предотвращая взломы и атаки. 
Он обеспечивает защиту от DDoS-атак, распределяя трафик и блокируя подозрительные запросы. 
Скрытие реального IP-адреса сервера делает его менее уязвимым. 
Прокси использует шифрование для конфиденциальности данных и защиты от перехвата. 
Контролирует доступ, устанавливая ограничения на IP-адреса и пользователей. Кэширует данные, ускоряя загрузку и снижая нагрузку на сервер, способствуя его защите от перегрузок и сбоев.

Прокси-серверы могут также использоваться для тестирования веб-сайтов и приложений. Они позволяют имитировать доступ из разных географических точек и проверять работу ресурсов в различных условиях.
___
...
ARP запрос (Address Resolution Protocol Request) - это сообщение, отправляемое устройством в компьютерной сети для получения физического (MAC) адреса другого устройства, имеющего определенный IP-адрес. Оно используется, когда устройство хочет отправить данные другому устройству, но не знает его физического адреса. ARP запрос отправляется широковещательно по всей сети, чтобы получить ответ от устройства, которому адресован запрос. Это позволяет устройствам установить связь и обмениваться данными в сети.

Запрос ARP (ARP Request): "Кто имеет определенный IP-адрес?" Оно отправляет широковещательный запрос по всей сети.

Ответ ARP (ARP Reply): Устройство с запрошенным IP-адресом отвечает на запрос, сообщая свой физический MAC-адрес.

Обновление ARP-кэша: Устройство, отправившее запрос, запоминает полученные данные. Теперь оно знает, какому физическому адресу соответствует нужный IP-адрес. ARP-адреса могут быть статическими или динамическими. Статические адреса устанавливаются вручную и не изменяются, а динамические адреса формируются автоматически в процессе обмена данными в сети.

### HTTP 
HTTP (Hypertext Transfer Protocol) используется для передачи гипертекстовых документов, что делает его основой для взаимодействия между веб-браузерами (клиентами) и веб-серверами. протокол который используется всякий раз когда клиент общается в веб сервером. на веб серверах размещено множество веб сайтов написанных на языке html который расшифровывается как язык разметки гипертекста. и обмен этими html страницами осуществляется по протоколу http. когда вы заходите на сайт vk.com ваш клиент(веб браузер) отправляет запрос get на веб сервер и вебсервер отвечает 200 ОК, затем укажите веб сайт который вы запрашиваете
___
### 55 DHCP 
протокол служит для автоматизации назначения IP-адреса клиенту. когда новое устройство подключается к сети оно отправляет сообщение на dhcp сервер с просьбой выдать свободный ip адресс, сервер при помощи mac адреса возвращает ответ в котором указывает назначенный ip адрес.

___
### 56 ICMP 
(Internet Control Message Protocol) протокол межсетевых управляющих сообщений используется для диагностики проблем со связностью в сети. помогает определить может ли достичь пакет адреса назначения в установленные временные рамки. Обычно, ICMP “юзают" устройства имеющие ip адрес (устройства 3 уровня)
для чего нужен?
+ если часть данных не доходит до адреса назначения, теряется или превышает допустимые таймауты - ICMP генерирует ошибки.

+ утилита ping. Термин “пинговать" как - раз связан с протоколом ICMP. “пинговать" хост - означает отправлять ICMP пакеты с целью понять, отвечает ли на них целевое устройство. инструмент инженера, который позволяет понять “"А жив ли хост?" и пинг может измерить задержку,
...
+ утилита traceroute. Когда говорят “сделайте трассировку маршрута" это означает, что мы хотим увидеть полный маршрут между хостом, на котором выполняется трассировка до хоста назначения. Трассировка покажет каждый из маршрутизаторов на пути до цели и время обработки и прохождения каждого из участков маршрута."Маршрут от отправителя до получателя называется хопом. Если на пути встречается 7 маршрутизаторов, говорят, что есть 7 хопов. Если пакет обрабатывается дольше обычного на 6 хопе, то в среде инженеров говорят “на 6 хопе повышенная задержка"
...
-(Internet Control Message Protocol) протокол межсетевых управляющих сообщений, служит для оповещений об ошибке при отправке данных (чтобы указать что пакет не доставлен) и для диагностики работоспособности сети, команды использующие ICMP - ping и traseroute(пример для windows - tracert vk.com)используется, например, для проверки, работает ли компьютер или сетевое устройство, а также для передачи сообщений об ошибках, которые могут возникнуть при отправке данных в сети. Протокол ICMP является частью базового набора протоколов Интернета и широко применяется для диагностики и управления сетевыми соединениями.
Протокол ICMP (Internet Control Message Protocol) может использоваться для следующих целей:

Проверка доступности узла: ICMP позволяет отправлять запросы эхо (ping) для проверки доступности узла в сети. Это позволяет определить, работает ли узел и насколько быстро он отвечает на запросы.
Обнаружение ошибок: ICMP используется для передачи сообщений об ошибках, которые могут возникнуть при отправке данных в сети. Например, если пакет данных не может быть доставлен до назначения, узел может отправить сообщение об ошибке ICMP, чтобы уведомить отправителя о проблеме.
Диагностика сети: ICMP может быть использован для диагностики и устранения проблем с сетевым соединением. Например, с помощью утилиты ping можно проверить, сколько времени требуется для отправки пакета данных до удаленного узла и получения ответа. Это может помочь определить проблемы сети, такие как задержки или потеря пакетов.
Управление сетевыми устройствами: ICMP также может использоваться для управления сетевыми устройствами, например, для отправки команд на маршрутизаторы или другие сетевые устройства.
Выполнение утилиты traceroute: ICMP может быть использован для выполнения утилиты traceroute. Traceroute используется для определения маршрута, который пакет данных проходит от отправителя к получателю через сеть. Он отправляет серию ICMP-запросов с постепенно увеличивающимся значением TTL (Time to Live) и анализирует ICMP-ответы, чтобы определить промежуточные узлы, через которые проходит пакет данных. Это может быть полезно для определения проблем с маршрутизацией или для определения самого быстрого маршрута для передачи данных.
...
___
### 59 DNS 
Domain Name System протокол который преобразует доменное имя в ip адрес
___
### 60 Маска подсети
___
### 61 TCP\UDP 
